...
import { Plus, Trash2, Search, Minus, ShoppingCart, User, ArrowLeft, FileText, Check, X } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-500 mt-1">Crie um orçamento para o cliente</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
                    ...
                    {filteredProducts.map(p => (
                      <tr key={p.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-3 py-2 text-center">
                          <button
                            onClick={() => addToCart(p)}
                            className="p-1.5 bg-primary-600 text-white rounded-lg hover:bg-primary-700 touch-manipulation"
                          >
          ...
          <div className="card">
            <h3 className="card-header flex items-center gap-2">
              <ShoppingCart className="w-5 h-5" /> Itens do Orçamento ({cart.length})
            </h3>
            {cart.length === 0 ? (
              <div className="text-center py-8 text-gray-400">
                <ShoppingCart className="w-12 h-12 mx-auto mb-2 opacity-50" />
                <p className="text-sm">Nenhum item adicionado. Selecione produtos acima.</p>
              </div>
            ) : (
        ...
        <div className="space-y-6">
          ...
          <div className="card">
            <h3 className="card-header">Configurações</h3>
            <div className="space-y-3">
              <div>
                <label className="block text-sm text-gray-600 mb-1">Validade do Orçamento</label>
          ...
          <div className="card">
            ...
            >
              {saving ? (
                ...
                <><FileText className="w-5 h-5" /> Gerar Orçamento</>
              )}
            </button>
          </div>
        </div>
